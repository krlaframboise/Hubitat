...
private getDriverDetails() { 
	return "<br>Dome Mouser<br>Version 0.0.0<br>Supported Devices: DMMZ1"
}
...
metadata {
	definition (
		...
		fingerprint deviceId: "0104", inClusters: "0x30,0x59,0x5A,0x5E,0x70,0x71,0x72,0x73,0x80,0x84,0x85,0x86", outClusters: "", mfr:"021F", prod:"0003", deviceJoinName: "Dome Mouser"		
	}
...
def configure() {
	...
	configData.sort { it.paramNum }.each { 
		cmds += updateConfigVal(it, it.value, refreshAll)	
	}
...
private configGetCmd(param) {
	return zwave.configurationV1.configurationGet(parameterNumber: param.paramNum).format()
}

private configSetCmd(param, val) {
	return zwave.configurationV1.configurationSet(parameterNumber: param.paramNum, size: param.size, configurationValue: convertFromScaledValue(val, param.size)).format()
}
...
private getConfigData() {
	// [paramNum: 1, name: "Basic Set Association Group 2", size: 1],
	// [paramNum: 2, name: "Firing Mode", size: 1],
	// [paramNum: 3, name: "High Voltage Duration Time", size: 2],
	return [		
		[paramNum: 4, name: "LED Alarm Enabled", value: (ledAlarmSetting == "Disabled") ? 0 : 1, size: 1],
		[paramNum: 5, name: "LED Alarm Length", value: convertOptionSettingToInt(ledAlarmOptions, ledAlarmSetting), size: 1]
	]	
}
