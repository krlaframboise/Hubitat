...
def notificationsPage() {
	dynamicPage(name:"notificationsPage") {
		section ("Notification Settings") {
			...
			input "pushDevices", "capability.notification", title: "Send notification to device(s)?", multiple: true, required: false			
			input "phone", "phone", title: "Phone number for SMS format +1<10 digits> (separate multiple with , )", required: false, submitOnChange: true
			input "modes", "mode", title: "Only send Notifications for specific mode(s)", multiple: true, required: false 			
	...
	dynamicPage(name: "enableDashboardPage", title: "") {
		section() {
			if (initializeAppEndpoint()) {
				...
				paragraph "Please open the Simple Device Viewer in the Apps Code section, click the 'OAuth', click the 'Enable OAuth' button, and then click the 'Update' button.\n\nOnce finished, tap Done and try again.", title: "Please enable OAuth for Simple Device Viewer", required: true, state: null
			}
		}
	}
}
...
private getSelectedCapabilitySettings(timeout) {	
	if (!settings.enabledCapabilities) {
		return capabilitySettings().findAll { 
			devicesHaveCapability(getCapabilityName(it)) 
		}
	}
	else {
		def startTime = new Date().time
...
private boolean canSendNotification(lastSent, repeatMS) {	
	if (!modes || modes.contains(location.mode)) {
		...
		logTrace "Ignore Notification because mode is ${location.mode}"
		return false
	}
}

private sendNotificationMessage(message) {	
	if (pushDevices || phone) {
		state.currentCheckSent = state.currentCheckSent + 1

		if (pushDevices)	{
			logInfo "Sending Notification to '${pushDevices}': ${message}"
			pushDevices.deviceNotification(message)
		}
		
		if (phone) {
			logInfo "Sending SMS to '${phone}': ${message}"
			"${phone}".tokenize(",")?.each {
				sendSms(it, message)
			}
		}
...
private api_dashboardUrl(capName=null) {	
	...
	return api_buildDashboardUrl("dashboard/${pageName}")	
}

private api_buildDashboardUrl(path, baseUrl=null) {
	if (!baseUrl) baseUrl = "${state.endpoint}"
	return "${baseUrl}".replace("?access_token", "${path}?access_token")
}
...
private api_getToggleItemsHtml(currentUrl, listItems) {
	...
	listItems.unique().each {		
		html += api_getItemHtml(it.title, it.image, api_buildDashboardUrl("/toggle/${it.deviceId}", currentUrl), it.deviceId, it.status)
	}
	...
	if (imageName in ["light", "switch"]) {
		html += api_getItemHtml("Turn All Off", "${imageName}-off all-command", api_buildDashboardUrl("/off", currentUrl), "", "")	
		
		html += api_getItemHtml("Turn All On", "${imageName}-on all-command", api_buildDashboardUrl("/on", currentUrl), "", "")
	}
	return html
}
